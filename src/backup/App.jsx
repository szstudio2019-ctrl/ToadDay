...
export default function App() {
  ...
  // Hero video: plays once scrolled into view, then slides diagonally off to the
  // left and fades out as the user scrolls into the "Our Story" section below it.
  useEffect(() => {
    const video = heroVideoRef.current;
    const heroEl = heroSectionRef.current;
    const storyEl = storySectionRef.current;
    const headingEl = storyHeadingRef.current;
    if (!video || !heroEl || !storyEl || !headingEl) return;

    const ctx = gsap.context(() => {
      // The hero is the first section on the page, so it's already in view on
      // load — there's no reachable "scroll down into it" boundary to cross.
      // Start playback immediately, then use ScrollTrigger purely to pause/resume
      // as the hero actually scrolls out of / back into view.
      video.play().catch(() => {});
      ...
      // Move the video down the Y axis and over to the left until it settles
      // right beside the "Our Story" heading. Targets are computed as functions
      // so GSAP re-measures the real, untransformed positions on every
      // ScrollTrigger refresh (e.g. on resize) instead of a guessed percentage.
      const endScale = 0.35;
      ...
      gsap.to(video, {
        x: () => {
          ...
          // Land the video's *scaled-down* right edge just to the left of the
          // heading, with a small gap — not the full-size (pre-scale) width.
          return hRect.left - vRect.left - vRect.width * endScale - 24;
        },
        y: () => {
          const vRect = video.getBoundingClientRect();
          const hRect = headingEl.getBoundingClientRect();
          return hRect.top - vRect.top;
        },
        scale: 0.35,
        ease: "none",
        scrollTrigger: {
  ...
  return (
    <div style={{ fontFamily: ff, direction: "ltr", color: "#1a1a1a", background: "#fff", width: "100%" }}>
      ...
      {/* ── HERO ── */}
      <div ref={heroSectionRef} style={{ position: "relative", minHeight: fluid(560, 793), display: "flex", alignItems: "center", overflow: "hidden" }}>
        <img src="/bg_hero2.jpg"
          alt="" style={{ position: "absolute", left: 0, right: 0, top: "-15%", width: "100%", height: "130%", objectFit: "cover", zIndex: 0, willChange: "transform" }}
          ref={heroImgRef}
        />
        {isDesktop && (
          <video ref={heroVideoRef} src="/toad_jump.mp4" muted loop playsInline preload="auto"
        ...
        <div style={{ position: "relative", zIndex: 2, width: "100%", maxWidth: 1280, margin: "0 auto", padding: `0 ${fluid(24, 96)}` }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: fluid(12, 16) }}>
            <button onClick={() => storySectionRef.current?.scrollIntoView({behavior:'smooth'})}
      ...
      {/* ── WHY TOAD ── */}
      <div ref={storySectionRef} style={{ background: "#FBF9F8", display: "flex", flexDirection: "column", alignItems: "center", padding: `${fluid(48, 60)} 0` }}>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", gap: 16, padding: `0 ${fluid(24, 96)}`, width: "100%", maxWidth: 1280, boxSizing: "border-box", marginBottom: fluid(24, 40) }}>
          ...
          <h2 ref={storyHeadingRef} style={{ fontWeight: 900, fontSize: fluid(32, 60), lineHeight: "1.1", color: "#006E1C", textAlign: "left", margin: 0, width: "100%" }}>From an Israeli initiative to a growing international day</h2>
        </div>
          ...
          {isDesktop && (
            <div style={{ position: "relative", width: "100%", maxWidth: 1176, height: 690, margin: "0 auto" }}>
              ...
              <video src="/magnific_subtle-slow-motion.-the-t_IfaxexJtvE.mp4" autoPlay muted loop playsInline
                style={{ position:"absolute", left:0, top:56, width:"100%", height:634, objectFit:"cover", zIndex:1, mixBlendMode:"multiply", borderRadius:16 }} />
            </div>
          )}
      ...
      {/* ── HOW TO HELP ── */}
      <div style={{ background:"#fff", display:"flex", flexDirection: layout("row", "column"), alignItems:"center", justifyContent:"center", gap: fluid(24, 48) }}>
        <div style={{ width: layout(480, "100%"), flexShrink:0, position:"relative", height: fluid(300, 420) }}>
          <div style={{ position:"absolute", inset:0, borderRadius:16, overflow:"hidden" }}>
            <img src="/1_umt04-5.jpg" alt="" style={{ width:"100%", height:"100%", objectFit:"cover", display:"block" }} />
          </div>
          <img src="/toad_life.png" alt="Toad sheltering under a leaf" style={{ position:"absolute", left:"-8%", top:"-18%", width:"116%", height:"136%", objectFit:"contain", zIndex:2 }} />
        </div>
        <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"flex-start", justifyContent:"center", gap:16, padding: `${fluid(40, 80)} ${fluid(24, 80)}` }}>
          ...
          <h2 style={{ fontWeight:900, fontSize: fluid(32, 56), lineHeight:"1.1", color:"#009D3A", textAlign:"left", margin:"0 0 16px", width:"100%" }}>Four Reasons to Appreciate Toads</h2>
          <div style={{ display:"flex", flexDirection:"column", gap:28, width:"100%" }}>
            {[["They Do Not Cause Warts","The bumpy texture of a toad's skin is part of its natural anatomy. The old claim that touching a toad causes human warts is a myth."],["Natural Pest Controllers","Toads feed on insects and other invertebrates and can be valuable neighbors in gardens and agricultural landscapes."],["Remarkable Resilience","Many toads can use terrestrial habitats and tolerate drier conditions better than many other amphibians."],["They Still Need Protection","Wetland loss, pesticide use, pollution, roads, disease and climate change threaten amphibian populations worldwide."]].map(([title,text])=>(
              <div key={title} style={{ display:"flex", flexDirection:"row", alignItems:"flex-start", gap:20, direction:"ltr" }}>
                <div style={{ width:40, height:40, borderRadius:9999, background:"#D9E6DA", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                  <svg width="18" height="18" viewBox="0 0 20 20" fill="none"><path d="M16.667 5L7.5 14.167 3.333 10" stroke="#006E1C" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                </div>
